...
import org.m1theo.tinkerforge.client.devices.temperatureirv2.TemperatureIRV2DeviceConfig;
import org.m1theo.tinkerforge.client.devices.temperatureirv2.TemperatureIRV2Bricklet;
import org.m1theo.tinkerforge.client.devices.DeviceType;
import org.m1theo.tinkerforge.client.devices.temperatureirv2.ChannelId;
import org.m1theo.tinkerforge.client.types.*;

import org.m1theo.tinkerforge.client.devices.temperatureirv2.ObjectTemperatureChannel;
import org.m1theo.tinkerforge.client.devices.temperatureirv2.AmbientTemperatureChannel;
...
/**
 * The {@link TemperatureIRV2BrickletHandler} is responsible for handling commands, which are
...
public class TemperatureIRV2BrickletHandler extends BaseThingHandler implements CallbackListener, DeviceAdminListener {

    private final Logger logger = LoggerFactory.getLogger(TemperatureIRV2BrickletHandler.class);
    private @Nullable TemperatureIRV2DeviceConfig config;
    private @Nullable BrickdBridgeHandler bridgeHandler;
    private @Nullable TemperatureIRV2Bricklet device;
    private @Nullable String uid;
    private boolean enabled = false;

    public TemperatureIRV2BrickletHandler(Thing thing) {
        super(thing);
    }
...
private void enable(){
    ...
    private void getobjectTemperature() {
        ...
        if (brickdBridgeHandler != null) {
            ...
            if (device != null) {
                TemperatureIRV2Bricklet device2 = (TemperatureIRV2Bricklet) device;
                ObjectTemperatureChannel channel = (ObjectTemperatureChannel) device2.getChannel("objectTemperature");
                Object newValue = channel.getValue();
                
                if (newValue instanceof DecimalValue) {
                    logger.debug("new value {}", newValue);
                    updateState(ChannelId.objectTemperature.name(), new QuantityType<>(new DecimalType(((DecimalValue) newValue).bigDecimalValue()), SIUnits.CELSIUS));
    ...
    private void getambientTemperature() {
        ...
        if (brickdBridgeHandler != null) {
            ...
            if (device != null) {
                TemperatureIRV2Bricklet device2 = (TemperatureIRV2Bricklet) device;
                AmbientTemperatureChannel channel = (AmbientTemperatureChannel) device2.getChannel("ambientTemperature");
                Object newValue = channel.getValue();
                
                if (newValue instanceof DecimalValue) {
                    logger.debug("new value {}", newValue);
                    updateState(ChannelId.ambientTemperature.name(), new QuantityType<>(new DecimalType(((DecimalValue) newValue).bigDecimalValue()), SIUnits.CELSIUS));
