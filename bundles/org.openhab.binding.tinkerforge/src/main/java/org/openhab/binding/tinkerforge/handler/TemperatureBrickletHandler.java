...
import org.m1theo.tinkerforge.client.devices.temperature.TemperatureChannelConfig;
...
/**
 * The {@link TemperatureBrickletHandler} is responsible for handling commands,
...
public class TemperatureBrickletHandler extends BaseThingHandler implements CallbackListener, DeviceAdminListener {

    private final Logger logger = LoggerFactory.getLogger(TemperatureBrickletHandler.class);
    private @Nullable TemperatureDeviceConfig config;
    ...
    public void initialize() {
        config = getConfigAs(TemperatureDeviceConfig.class);
            ...
            if (bridgeStatus == ThingStatus.ONLINE) {
                ...
                if (deviceIn != null) {
                    if (deviceIn.getDeviceType() == DeviceType.temperature) {
                        TemperatureBricklet device = (TemperatureBricklet) deviceIn;
